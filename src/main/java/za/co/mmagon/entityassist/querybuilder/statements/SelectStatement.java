		...
		if (sql != null)
		{
			...
			if (parameters != null && parameters.length > 0)
			{
				for (Object val : parameters)
				{
					...
					{
						value = STRING_SINGLE_QUOTES + val + STRING_SINGLE_QUOTES;
					}
					...
					{
						value = STRING_SINGLE_QUOTES + ((Class) val).getCanonicalName() + STRING_SINGLE_QUOTES;
					}
					else if (val instanceof Date)
					{
						SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss.SSS");
						value = STRING_SINGLE_QUOTES + sdf.format((Date) val) + STRING_SINGLE_QUOTES;
					}
					else if (val instanceof LocalDate)
					{
						SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
						value = STRING_SINGLE_QUOTES + sdf.format(val) + STRING_SINGLE_QUOTES;
					}
